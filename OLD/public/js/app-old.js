import { 
  ...
  getComicsList, getAudiobooksList,
  ...
  if (user) {
    ...
    try {
      ...
      if (isAuthorized) {
        ...
        // Mostrar menú de categorías en lugar de ir directo a libros
        showCategoryMenu();
...
// La UI inicial ya se controla desde el script inline del HTML
// Este listener solo se encarga de la lógica sin modificar la UI inicial
...
const showBooksSection = async () => {
  ...
  // Actualizar email del usuario en el header de libros
  const userEmailElement = document.getElementById('user-email');
  if (userEmailElement && currentUser) {
    userEmailElement.textContent = currentUser.email;
  }
  
  // Inicializar la sección de libros si no se ha hecho
  if (!booksInitialized) {
    loading(true);
    await initializeApp();
    await updateHistoryDisplay();  // Usar la función existente
  ...
  try {
    ...
    if (booksCount) {
      try {
        ...
        booksCount.textContent = booksList ? booksList.length.toLocaleString() : '0';
      } catch (error) {
        console.warn('Error obteniendo libros:', error);
    ...
    if (comicsCount) {
      try {
        const comicsList = await getComicsList();
        comicsCount.textContent = comicsList ? comicsList.length.toLocaleString() : '0';
      } catch (error) {
        console.warn('Cómics no configurados aún, mostrando 0');
        comicsCount.textContent = '0';
      }
    }
    
    // Contador de audiolibros
    const audiobooksCount = document.getElementById('audiobooks-count');
    if (audiobooksCount) {
      try {
        const audiobooksList = await getAudiobooksList();
        audiobooksCount.textContent = audiobooksList ? audiobooksList.length.toLocaleString() : '0';
      } catch (error) {
        console.warn('Audiolibros no configurados aún, mostrando 0');
        audiobooksCount.textContent = '0';
      }
    }
  } catch (error) {
...
const getBook = async (bookPath, startChapter = 0) => {
  ...
  // Estrategia híbrida: Intentar Nginx primero, luego Firebase Storage
  const nginxStorageUrl = 'https://storage.lecturapp.es';
  const nginxUrl = `${nginxStorageUrl}/LIBROS/${encodeURIComponent(bookPath)}`;
  
  console.log('📚 Buscando libro:', bookPath);
  console.log('🔗 Intentando Nginx primero:', nginxUrl);
  
  // 1. Intentar Nginx primero
  fetch(nginxUrl, {
    method: "GET"
  })
  .then(response => {
    if (response.ok) {
      console.log('✅ Encontrado en Nginx');
      return response.blob();
    } else {
      console.log('⚠️ No encontrado en Nginx, intentando Firebase Storage...');
      throw new Error('NotFoundInNginx');
    }
  })
  .then(blob => {
    loading(false);
  ...
  .catch(error => {
    if (error.message === 'NotFoundInMinIO') {
...
function initializeEventListeners() {
  ...
  // Event listener para el tema en el menú
  ...
  clickableLogos.forEach(logo => {
    logo.addEventListener('click', () => {
      // Ocultar sección actual y mostrar menú de categorías
      const mainContent = document.getElementById('main');
      const audiobooksPage = document.getElementById('audiobooks-grid');
      const comicsPage = document.getElementById('comics-grid');
      
      if (mainContent) mainContent.style.display = 'none';
      
      // Si estamos en audiolibros o cómics, volver a la página principal
      if (window.location.pathname.includes('audiobooks') || window.location.pathname.includes('comics')) {
